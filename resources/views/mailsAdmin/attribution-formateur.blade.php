...
<body>
  <table style="width:100%;max-width:960px;position: relative;left:0;right:0;margin: 0 auto;border-collapse: collapse;border-spacing: 0;font-size: 14px;line-height: 24px;color: #333;font-family: Microsoft YaHei;">
    ...
    <tr>
      <td style="padding: 20px 7.5% 0;">Un étudiant vient d'être attribué à un nouveau formateur !</td>
    </tr>

    <tr>
      <td style="padding: 20px 7.5% 0;"> Voici les infos de la relation <br>
        Etudiant: <strong>{{$user->name}}</strong> <br>
        Mail étudiant: <strong>{{$user->email}}</strong> <br>
        Formateur: <strong>{{$user->teacher->name}}</strong> <br>
        Mail formateur: <strong>{{$user->teacher->email}}</strong> <br>
      </td>
    </tr>
