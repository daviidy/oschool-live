@extends('layouts.menu-dashboard-factures')
@section('titre', 'Factures de '.Auth::user()->name)
@section('content')
...
<div id="state" class="box content">
    ...
    <section class="setting-linked-accounts--content-container--3172B form--flex-column--g8OLQ">
        <div class="setting-linked-accounts--main--1H5vk form--main--31YLe">
            <h1 class="setting-linked-accounts--content-header--2s99F form--header-text--ZDMyT">Mois: {{$mois}}-{{$an}}</h1>
            <ul>
                <li>
                    <dl>
                        <dd title="Connected" class="setting-linked-accounts--connected--2gX8u"><span>Effectué</span></dd><dt>{{ $classrooms->count() }} sessions</dt>
                        <dd> {{ $classrooms->count() }} * 3000 FCFA = </dd>
                        <dd> {{ 3000 * $classrooms->count() }} FCFA</dd>
